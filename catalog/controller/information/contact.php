...
class ControllerInformationContact extends Controller {
    ...
    public function ajaxSend() {
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            $mail->setTo($this->config->get('config_email'));
            $mail->setFrom($this->config->get('config_email'));
            $mail->setReplyTo('autopartsinvolved@gmail.com');
            $mail->setSender("Autoparts Callback", ENT_QUOTES, 'UTF-8');
            $mail->setSubject(html_entity_decode(sprintf(('Зворотній дзвінок'), ENT_QUOTES, 'UTF-8')));
    
            $html = '';
    
            $html .= '<p><b>Потрібно проконсультувати клієнта, він залишив номер телефону. </b></p>';
            $html .= '<p><b>Телефон:</b> '.$this->request->post['number'].'</p>';
    
            $mail->setHTML($html);
            $mail->send();
    
            $json['success'] = 'Дякуємо, найближчим часом з вами зв`яжуться';
